...
Sejak Fashion Pawrade Competition 2026 ditambahkan, `index.html` jadi halaman pilihan acara (Pet Blessing / Fashion Pawrade Competition 2026), dan form Pet Blessing sendiri pindah ke `petblessing.html`. Fashion Pawrade Competition 2026 (costume parade, bagian dari Colorful Carnival 2026) memakai struktur data dan kode yang sama persis (form, rekap, check-in) tapi tabel terpisah (`api.pawrade_*`, lihat `vps-db/init/12-pawrade.sql`) dan check-in satu titik saja (bukan 4 pos). Bot WhatsApp yang sama memproses kedua antrian.
