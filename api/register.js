...
// Satu-satunya jalur menulis data pendaftaran. Client TIDAK insert langsung
// ke PostgREST lagi (role web_anon sudah dicabut izin insert-nya, lihat
// vps-db/init/04-captcha-gate.sql) -- semua registrasi wajib lewat sini,
// supaya bisa diverifikasi captcha dulu sebelum tersimpan.
//
// Alur: verifikasi token Cloudflare Turnstile -> kalau lolos, terbitkan JWT
// pendek berumur role "web_registrant" -> pakai JWT itu untuk insert ke
// PostgREST atas nama client.
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    await insertRow('/pets', body.pets.map(function (p) {
      return {
        id: p.id,
        owner_id: p.owner_id,
        name: p.name,
    ...
    if (body.waQueue) {
      ...
      try {
        await insertRow('/wa_queue', body.waQueue);
      } catch (e) {
        console.error('wa_queue insert failed', e.message);
      }
    }
