...
Untuk ditampilkan di layar kedua atau monitor besar: tekan tombol **Buka layar monitor** di halaman check in (atau buka https://petblessings.vercel.app/monitor-checkin.html langsung, login superadmin). Layar dibagi dua: **kiri Pos A (nomor ganjil), kanan Pos B (nomor genap)**. Tiap sisi menampilkan nomor urut terbaru dalam ukuran besar, nama pemilik, nama dan jenis hewan, jumlah pendamping, beberapa reg ulang sebelumnya, progres "X dari Y nomor sudah reg ulang", dan **Berikutnya dipanggil** (nomor terkecil di sisi itu yang belum reg ulang dan lebih besar dari nomor tertinggi yang sudah reg ulang), plus **Terlewat** untuk nomor yang belum hadir padahal nomor di atasnya sudah. Ini membantu memanggil nomor berurutan di ruang tunggu: satu antrean ganjil untuk Pos A, satu antrean genap untuk Pos B. Angka "Berikutnya dipanggil" juga tampil di halaman scanner. Nomor HP tidak ditampilkan.
